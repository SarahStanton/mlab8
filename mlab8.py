...
def on_intent(intent_request, session):
    ...
    if intent_name == "CityIsIntent":
        return set_city_in_session(intent, session)
    elif intent_name == "StateIsIntent":
        return set_state_in_session(intent, session)
    elif intent_name == "WhatsMyWeatherIntent":
...
def set_city_in_session(intent, session):
    ...
    if 'City' in intent['slots']:
        selected_city = intent['slots']['City']['value']
        session_attributes['city'] = selected_city
...
def set_state_in_session(intent, session):
    ...
    if 'State' in intent['slots']:
        selected_state = intent['slots']['State']['value']
        session_attributes['state'] = selected_state
        speech_output = "I now know your state is " + \
                        selected_state + \
                        ". You can ask me what your weather is by saying, " \
                        "what is the weather?"
        reprompt_text = "You can ask me what your weather is by saying, " \
                        "what is the weather?"
    else:
...
def get_weather_from_session(intent, session):
    ...
    if session_attributes and "city" in session_attributes and "state" in session_attributes:
        ...
        speech_output = "Your current city is " + city + \
                        ". The current weather is " + w + \
                        ". Goodbye"
        should_end_session = True
    else:
        speech_output = "I'm not sure what your city or state is. " \
                        "You can tell me your city and state by saying, " \
                        "my city is Seattle, and then saying, my state is Washington."
        should_end_session = False
